...
Contained Trap::: The trap is visible to, and handled by, software running inside the
...
mode on the hart.
Requested Trap::: The trap is a synchronous exception that is an explicit call to the
...
Invisible Trap::: The trap is handled transparently by the execution environment and
