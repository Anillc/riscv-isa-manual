...
- A _Vector Element Group_ is a vector of one or more element groups.
...
\| Vd  | input  | 4_SEW  | 4 | SEW | current state {c, d, g, h}
\| Vs1 | input  | 4_SEW  | 4 | SEW | MessageSched plus constant[3:0]
\| Vs2 | input  | 4_SEW  | 4 | SEW | current state {a, b, e, f}
\| Vd  | output | 4_SEW  | 4 | SEW | next state {a, b, e, f}
|===
...
foreach (i from eg_start to eg_len-1) {
let {a @ b @ e @ f} : bits(4_SEW) = get_velem(vs2, 4_SEW, i);
let {c @ d @ g @ h} : bits(4_SEW) = get_velem(vd, 4_SEW, i);
let MessageShedPlusC[3:0] : bits(4_SEW) = get_velem(vs1, 4_SEW, i);
...
\| Vd  | input  | 4_SEW  | 4 | SEW | Message words {W[3],  W[2],  W[1],  W[0]}
\| Vs2 | input  | 4_SEW  | 4 | SEW | Message words {W[11], W[10], W[9],  W[4]}
\| Vs1 | input  | 4_SEW  | 4 | SEW | Message words {W[15], W[14], -, W[12]}
\| Vd  | output | 4_SEW  | 4 | SEW | Message words {W[19], W[18], W[17], W[16]}
|===
...
foreach (i from vstart to vl - 1) {
set_velem(vd, EEW=2_SEW, i,
get_velem(vs2, i) << (get_velem(vs1, i) & ((2_SEW)-1))
)
}
RETIRE_SUCCESS
}
...
foreach (i from vstart to vl - 1) {
set_velem(vd, EEW=2_SEW, i,
get_velem(vs2, i) << (uimm[4:0] & ((2_SEW)-1))
)
}
RETIRE_SUCCESS
}
...
\|        | | | |            |        | | |             |        | | |
\| 101000 | | | |            | 101000 |V| | _VAES.vv_   | 101000 | | |\
\| 101001 | | | |            | 101001 |V| | _VAES.vs_   | 101001 | | |\
...
OP-V (0x57)
_Zvbb_, _Zvkb_, and _Zvbc_ Vector instructions _in bold_
//[%auto-width]
...
\| 000001 |V|X| | _vandn_    | 000001 |V| | vredand     | 000001 |V| | vfredusum
...
\| 001100 |V|X|I| vrgather   | 001100 |V|X| _vclmul_    | 001100 | | |
\| 001101 | | | |            | 001101 |V|X| _vclmulh_   | 001101 | | |
...
\| 010100 |V|X| | _vror_     | 010100 |V| | VMUNARY0    | 010100 | | |
\| 010101 |V|X| | _vrol_     | 010101 | | |             | 010101 | | |
\| 01010x | | |I| _vror_     |        | | |             |        | | |
...
\| 110101 |V|X|I| _vwsll_    | 110101 |V|X| vwadd.w     | 110101 | | |
